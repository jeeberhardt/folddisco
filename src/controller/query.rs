...
pub fn make_query_map(
    path: &String, query_residues: &Vec<(Vec<u8>, u64)>, hash_type: HashType, 
    nbin_dist: usize, nbin_angle: usize, multiple_bin: &Option<Vec<(usize, usize)>>,
...
) -> (HashMap<GeometricHash, ((usize, usize), bool)>, Vec<usize>, HashMap<(Vec<u8>, Vec<u8>), Vec<(f32, usize)>>) {

    let (compact, _) = read_compact_structure(path).expect("Failed to read compact structure");
    
    let mut hash_collection = HashMap::new();
    let mut observed_distance_map: HashMap<(Vec<u8>, Vec<u8>), Vec<(f32, usize)>> = HashMap::new();
    ...
    comb_iter.for_each(|(i, j)| {
        ...
        if is_feature {
            ...
            if let Some(aa_dist_info) = aa_dist_info {
                ...
                let aa_pair = (vec![aa_dist_info.0], vec![aa_dist_info.1]);
...
pub fn parse_query_string(query_string: &str, mut default_chain: Vec<u8>) -> (Vec<(Vec<u8>, u64)>, Vec<Option<Vec<u8>>>) {
    ...
    if default_chain.is_empty() || !default_chain[0].is_ascii_alphabetic() {
        default_chain = vec![b'A'];
    }
    // Remove whitespace
    let query_string = query_string.replace(" ", "");
    for segment in query_string.split(',') {
        // Support formats like 1.A:124 or 1.A:124:H or 1.BB:4:RX
        let (chain, residue_num, subst_part) = if let Some((chain_part, rest)) = segment.split_once(':') {
            let (residue_str, subst_str) = match rest.split_once(':') {
                Some((res, subst)) => (res, Some(subst)),
                None => (rest, None),
            };
            let residue_num = residue_str.parse::<u64>().unwrap_or_else(|_| panic!("Invalid residue: {}", residue_str));
            let subst = subst_str.map(|s| {
                s.chars()
                    .filter(|c| is_aa_group_char(*c))
                    .flat_map(|c| map_one_letter_to_u8_vec(c))
                    .collect::<Vec<_>>()
            });
            (chain_part.as_bytes().to_vec(), residue_num, subst)
        } else {
            // Fallback to old logic
            let (chain, rest) = if let Some(first) = segment.chars().next() {
                if first.is_ascii_alphabetic() {
                    (first.to_string().as_bytes().to_vec(), &segment[first.len_utf8()..])
                } else {
                    (default_chain.clone(), segment)
                }
            } else {
                (default_chain.clone(), segment)
            };
            ...
            if range_part.contains('-') {
                ...
                let residue_num = range_part.parse::<u64>().unwrap_or_else(|_| panic!("Invalid residue: {}", range_part));
                (chain, residue_num, subst_part)
            }
        };
...
mod tests {
    ...
    fn test_make_query_map() {
        ...
        let query_residues = vec![
            (vec![b'F'], 207), (vec![b'F'], 212), (vec![b'F'], 225)
        ];
    ...
    fn test_parse_query_string() {
        ...
        let query_residues = parse_query_string(query_string, vec![b'A']);
        assert_eq!(query_residues, (vec![(vec![b'A'], 250), (vec![b'B'], 232), (vec![b'C'], 269)], vec![None, None, None]));
    }
    #[test]
    fn test_parse_query_string_with_space() {
        let query_string = "A250, A232, A269";
        let query_residues = parse_query_string(query_string, vec![b'A']);
        assert_eq!(query_residues, (vec![(vec![b'A'], 250), (vec![b'A'], 232), (vec![b'A'], 269)], vec![None, None, None]));
    }
    ...
    fn test_parse_query_string_with_aa_substitution() {
        ...
        let query_residues = parse_query_string(query_string, vec![b'A']);
        // R = 1, K = 11, Q = 5
        assert_eq!(query_residues, (vec![(vec![b'A'], 250), (vec![b'B'], 232), (vec![b'C'], 269)], vec![Some(vec![1]), Some(vec![11]), Some(vec![5, 11])]));
        let query_string = "250:R,232:K,269:QK";
        let query_residues = parse_query_string(query_string, vec![b'A']);
        // R = 1, K = 11, Q = 5
        assert_eq!(query_residues, (vec![(vec![b'A'], 250), (vec![b'A'], 232), (vec![b'A'], 269)], vec![Some(vec![1]), Some(vec![11]), Some(vec![5, 11])]));
    }
    #[test]
    fn test_parse_query_string_with_range() {
        ...
        assert_eq!(query_residues, (vec![
            (vec![b'A'], 250), (vec![b'A'], 251), (vec![b'A'], 252), 
            (vec![b'B'], 232), (vec![b'B'], 233), (vec![b'B'], 234), 
            (vec![b'C'], 269),
